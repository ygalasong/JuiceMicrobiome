...
oj_shannon <- div_iNEXT %>% 
  ...
  geom_dotplot(binaxis='y', stackdir = "center",position=position_dodge(0.9), binwidth = 2)+
  scale_fill_manual(values = Thermal_color)+
...
cd_shannon <- div_iNEXT %>% 
  ...
  ggplot(aes(x = Shelf_Life_Stage, y = Observed, fill = Thermal))+
  geom_boxplot(alpha = 0.75) + 
  geom_dotplot(binaxis='y', stackdir = "center",position=position_dodge(0.9), binwidth =1)+
  scale_fill_manual(values = Thermal_color)+
  theme_classic()+labs(x = "Stage of Shelf-life", y = "Shannon diversity")+
  theme(legend.position = "bottom")
```
...
ggpubr::ggarrange(oj_shannon, oj_simpson, cd_shannon, cd_simpson, 
                  ...
                  labels = c("A", "B", "C", "D"))
ggsave(filename = "Figure/shannon_simpson_ITS.png", device = "png", height = 5, width = 7, units = "in")
```
##### Session information
```{r}
