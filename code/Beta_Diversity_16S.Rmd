...
```{r}
pacman::p_load(phyloseq, tidyverse, ggpubr, ggforce, tempted, vegan,
...
```{r}
source(file = "/local/workdir/yg225/JuiceMicrobiome/code/colors.R")
```
...
Clearly, the beta diversity is driven by Trial, i.e. the source of the juice.We can also kind of see that samples form thermal VS non-thermal
...
To determine the statistical differences between groups, we can run PERMANOVA on the TEMPTED output.
```{r}
datlist_oj_16S <- format_tempted(counttable_oj,
                                 metatable_oj$Day,
                                 metatable_oj$JuiceID,
                                 threshold = 0.95,
                                 transform = "none")
ratio_feat_oj_16S <- ratio_feature(res_count_oj, datlist_oj_16S, pct = 0.005, absolute = TRUE)

## summed up, by individual subject
tab_feat_ratio_oj_16S <- ratio_feat_oj_16S$metafeature_ratio
colnames(tab_feat_ratio_oj_16S )[2] <- 'JuiceID'
tab_feat_ratio_oj_16S <- merge(tab_feat_ratio_oj_16S, metauni)

## summed up, by mean and sd
reshape_feat_ratio_oj_16S <- reshape(tab_feat_ratio_oj_16S, 
                            idvar=c("JuiceID","timepoint") , 
                            v.names=c("value"), timevar="PC",
                            direction="wide")
CC <- grep("value", colnames(reshape_feat_ratio_oj_16S))
colnames(reshape_feat_ratio_oj_16S)[CC] <- paste("Component", 1:length(CC))
feature_mat_ratio_oj_16S <- reshape_feat_ratio_oj_16S[,c("Component 1", "Component 2", "Component 3")]

time_vec_ratio <- reshape_feat_ratio_oj_16S$timepoint
group_vec_ratio <- factor(reshape_feat_ratio_oj_16S$Tech, 
                        levels=c("Untreated & UV", "Pasteurization", "HPP"))
## visualize metafeature ratios
p_feat_ratio_summary_oj_16S <- plot_feature_summary(feature_mat_ratio_oj_16S, 
                                           time_vec_ratio, 
...
permanova_oj_16S <- adonis2(reshape_feat_ratio_oj_16S[,8:10] ~ Tech,
                            method = "euclidean",
                            data = reshape_feat_ratio_oj_16S) 

# Can component 1 & 2 discriminate juice from different trial?
permanova_oj_16S_trial <- adonis2(reshape_feat_ratio_oj_16S[,8:10] ~ Trial,
                            method = "euclidean",
                            data = reshape_feat_ratio_oj_16S) 
...
```
Plot PPCs relevant to Component 1
```{r}
...
To determine the statistical differences between groups, we can run PERMANOVA on the TEMPTED output.
```{r}
datlist_cd_16S <- format_tempted(counttable_cd,
                                 metatable_cd$Day,
                                 metatable_cd$JuiceID,
                                 threshold = 0.95,
                                 transform = "none")
ratio_feat_cd_16S <- ratio_feature(res_count_cd, datlist_cd_16S, pct = 0.005, absolute = TRUE)

## summed up, by individual subject
tab_feat_ratio_cd_16S <- ratio_feat_cd_16S$metafeature_ratio
colnames(tab_feat_ratio_cd_16S )[2] <- 'JuiceID'
tab_feat_ratio_cd_16S <- merge(tab_feat_ratio_cd_16S, metauni)

## summed up, by mean and sd
reshape_feat_ratio_cd_16S <- reshape(tab_feat_ratio_cd_16S, 
                            idvar=c("JuiceID","timepoint") , 
                            v.names=c("value"), timevar="PC",
                            direction="wide")
CC <- grep("value", colnames(reshape_feat_ratio_cd_16S))
colnames(reshape_feat_ratio_cd_16S)[CC] <- paste("Component", 1:length(CC))
feature_mat_ratio_cd_16S <- reshape_feat_ratio_cd_16S[,c("Component 1", "Component 2", "Component 3")]

time_vec_ratio <- reshape_feat_ratio_cd_16S$timepoint
group_vec_ratio <- factor(reshape_feat_ratio_cd_16S$Tech, 
                        levels=c("Untreated & UV", "Pasteurization", "HPP"))
## visualize metafeature ratios
p_feat_ratio_summary_cd_16S <- plot_feature_summary(feature_mat_ratio_cd_16S, 
                                           time_vec_ratio, 
                                           group_vec_ratio, bws=30, nrow=1) + 
  xlab('Day') + theme_pubr() + 
  theme(legend.position='bottom') +
...
## PERMANOVA
# Can component 2 & 3 discriminate juice processed by different technology?
permanova_cd_16S <- adonis2(reshape_feat_ratio_cd_16S[,8:10] ~ Tech,
                            method = "euclidean",
                            data = reshape_feat_ratio_cd_16S) 

# Can component 1 - 3 discriminate juice from different trial?
permanova_cd_16S_trial <- adonis2(reshape_feat_ratio_cd_16S[,8:10] ~ Trial,
                            method = "euclidean",
                            data = reshape_feat_ratio_cd_16S) 
