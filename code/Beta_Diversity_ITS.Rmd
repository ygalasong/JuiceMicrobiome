...
metadata <- PERFect_physeq %>% sample_data() %>% data.frame() %>% rownames_to_column(var = "Assemblage")
...
metauni <- unique(metatable[,c("JuiceID","Beverage","Treatment","Trial","Tech")])
```
...
p_oj <- A_data_oj %>% 
  ...
  geom_point(aes(color = Trial, shape = Tech), size = 4)+ 
...
```{r}
# Plot of subject loading
p_oj2 <- A_data_oj %>% ggplot(aes(x = PC1, y = PC2))+
  geom_point(aes(color = Tech, shape = Trial), size = 4)+ 
  labs(x = "Component 1", 
       y = "Component 2",
       title = "Subject loadings")+
  scale_color_manual(values = Tech_color)+
  geom_mark_ellipse(aes(fill = Tech), alpha = 0.15,
                          expand = unit(2, "mm"))+
...
PERMANOVA
```{r}
datlist_oj_ITS <- format_tempted(counttable_oj,
                                 metatable_oj$Day,
                                 metatable_oj$JuiceID,
                                 threshold = 0.95,
                                 transform = "none")
ratio_feat_oj_ITS <- ratio_feature(res_count_oj, datlist_oj_ITS, pct = 0.005, absolute = TRUE)

## summed up, by individual subject
tab_feat_ratio_oj_ITS <- ratio_feat_oj_ITS$metafeature_ratio
colnames(tab_feat_ratio_oj_ITS )[2] <- 'JuiceID'
tab_feat_ratio_oj_ITS <- merge(tab_feat_ratio_oj_ITS, metauni)

## summed up, by mean and sd
reshape_feat_ratio_oj_ITS <- reshape(tab_feat_ratio_oj_ITS, 
                            idvar=c("JuiceID","timepoint") , 
                            v.names=c("value"), timevar="PC",
                            direction="wide")
CC <- grep("value", colnames(reshape_feat_ratio_oj_ITS))
colnames(reshape_feat_ratio_oj_ITS)[CC] <- paste("Component", 1:length(CC))
feature_mat_ratio_oj_ITS <- reshape_feat_ratio_oj_ITS[,c("Component 1", "Component 2", "Component 3")]

time_vec_ratio <- reshape_feat_ratio_oj_ITS$timepoint
group_vec_ratio <- factor(reshape_feat_ratio_oj_ITS$Tech, 
                        levels=c("Untreated & UV", "Pasteurization", "HPP"))
## visualize metafeature ratios
p_feat_ratio_summary_oj_ITS <- plot_feature_summary(feature_mat_ratio_oj_ITS, 
                                           time_vec_ratio, 
                                           group_vec_ratio, bws=30, nrow=1) + 
  xlab('Day') + theme_pubr() + 
  theme(legend.position='bottom') +
  scale_color_manual(values=Tech_color) + scale_fill_manual(values=Tech_color)

## PERMANOVA
# Can component 2 & 3 discriminate juice processed by different technology?
permanova_oj_ITS <- adonis2(reshape_feat_ratio_oj_ITS[,7:9] ~ Tech,
                            method = "euclidean", 
                            strata = reshape_feat_ratio_oj_ITS$Trial,
                            data = reshape_feat_ratio_oj_ITS) 

# Can component 1 & 2 discriminate juice from different trial?
permanova_oj_ITS_trial <- adonis2(reshape_feat_ratio_oj_ITS[,7:9] ~ Trial,
                            method = "euclidean",
                            data = reshape_feat_ratio_oj_ITS) 
...
```{r}
trial_legend <- get_legend(p_oj_min1)
tech_legend <- get_legend(p_oj_min3)

TEMPTED_ITS_OJ <- ggarrange(ggarrange(temporal_oj, ggarrange(p_oj_min1, p_oj_min3, ncol = 2, legend.grob = trial_legend, legend = "bottom"), nrow = 2, labels = c("A","B"), legend = "top"), topfeat_oj, ncol = 2, labels = c("","C"), legend.grob = tech_legend, legend = "bottom")
```
...
```{r}
comp3_PPCs <- plot_feature_summary(proportion_table_oj %>% 
                       select(ASV_1, ASV_4, ASV_3, ASV_15, ASV_47, ASV_48), 
                     metatable_oj$Day, 
                     metatable_oj$Tech, 
                     bws=30, nrow = 3) + 
...
p_cd <- A_data_cd %>% 
  ggplot(aes(x = PC1, y = PC2))+
  geom_point(aes(shape = Tech, color = Treatment), 
...
```{r}
# Plot of subject loading
p_cd2 <- A_data_cd %>% ggplot(aes(x = PC1, y = PC2))+
  geom_point(aes(shape = Tech, color = Treatment), size = 4)+ 
  labs(x = "Component 1", 
       y = "Component 2",
       title = "Subject loadings")+
  scale_color_manual(values = Treatment_color)

p_cd2 + geom_mark_ellipse(aes(fill = Tech), alpha = 0.15,
                          expand = unit(0, "mm"))+
  scale_fill_manual(values = Tech_color)+
  scale_y_continuous(limits = c(-0.4,0.8))+
  scale_x_continuous(limits = c(-0.6,0.6))+
...
PERMANOVA
```{r}
datlist_cd_ITS <- format_tempted(counttable_cd,
                                 metatable_cd$Day,
                                 metatable_cd$JuiceID,
                                 threshold = 0.95,
                                 transform = "none")
ratio_feat_cd_ITS <- ratio_feature(res_count_cd, datlist_cd_ITS, pct = 0.005, absolute = TRUE)

## summed up, by individual subject
tab_feat_ratio_cd_ITS <- ratio_feat_cd_ITS$metafeature_ratio
colnames(tab_feat_ratio_cd_ITS )[2] <- 'JuiceID'
tab_feat_ratio_cd_ITS <- merge(tab_feat_ratio_cd_ITS, metauni)

## summed up, by mean and sd
reshape_feat_ratio_cd_ITS <- reshape(tab_feat_ratio_cd_ITS, 
                            idvar=c("JuiceID","timepoint") , 
                            v.names=c("value"), timevar="PC",
                            direction="wide")
CC <- grep("value", colnames(reshape_feat_ratio_cd_ITS))
colnames(reshape_feat_ratio_cd_ITS)[CC] <- paste("Component", 1:length(CC))
feature_mat_ratio_cd_ITS <- reshape_feat_ratio_cd_ITS[,c("Component 1", "Component 2", "Component 3")]

time_vec_ratio <- reshape_feat_ratio_cd_ITS$timepoint
group_vec_ratio <- factor(reshape_feat_ratio_cd_ITS$Tech, 
                        levels=c("Untreated & UV", "Pasteurization", "HPP"))
## visualize metafeature ratios
p_feat_ratio_summary_cd_ITS <- plot_feature_summary(feature_mat_ratio_cd_ITS, 
                                           time_vec_ratio, 
                                           group_vec_ratio, bws=30, nrow=1) + 
  xlab('Day') + theme_pubr() + 
  theme(legend.position='bottom') +
...
## PERMANOVA
# Can component 2 & 3 discriminate juice processed by different technology?
permanova_cd_ITS <- adonis2(reshape_feat_ratio_cd_ITS[,7:9] ~ Tech,
                            method = "euclidean", 
                            strata = reshape_feat_ratio_cd_ITS$Trial,
                            data = reshape_feat_ratio_cd_ITS) 

# Can component 1 & 2 discriminate juice from different trial?
permanova_cd_ITS_trial <- adonis2(reshape_feat_ratio_cd_ITS[,7:9] ~ Trial,
                            method = "euclidean",
                            data = reshape_feat_ratio_cd_ITS) 
```
...
combine TEMPTED results
```{r}
trial_legend <- get_legend(p_cd_min1)
tech_legend <- get_legend(p_cd_min3)

TEMPTED_ITS_cd <- ggarrange(ggarrange(temporal_cd, ggarrange(p_cd_min1, p_cd_min3, ncol = 2, legend.grob = trial_legend, legend = "bottom"), nrow = 2, labels = c("A","B"), legend = "top"), topfeat_cd, ncol = 2, labels = c("","C"), legend.grob = tech_legend, legend = "bottom")
...
```{r}
pc1_highlights_cd <- plot_feature_summary(proportion_table_cd %>% 
                       select(ASV_8, ASV_23, ASV_24, ASV_34, ASV_35,ASV_50, ASV_42), 
                     metatable_cd$Day, 
                     metatable_cd$Tech, 
                     bws=30, nrow = 3) + 
  labs(x="Day", y="Relative Abundance")+
...
pc1_mrakia <- plot_feature_summary((proportion_table_cd %>% select(ASV_23,ASV_34,ASV_42)), metatable_cd$Day, metatable_cd$Tech, bws = 30, nrow = 3)+
  labs(x = "Day", y = "Relative Abundance")+
  scale_fill_manual(values = Tech_color)+
  scale_color_manual(values = Tech_color)+
  facet_wrap(vars(feature_all), ncol = 1,
             scales = "free_y", labeller = 
               labeller(feature_all = top_abs_PC1_cd_labs))+theme_pubr(legend = "none")

pc1_pichia <- plot_feature_summary((proportion_table_cd %>% select(ASV_8,ASV_35,ASV_50)), metatable_cd$Day, metatable_cd$Tech, bws = 30, nrow = 3)+
  labs(x = "Day", y = "Relative Abundance")+
  scale_fill_manual(values = Tech_color)+
  scale_color_manual(values = Tech_color)+
  facet_wrap(vars(feature_all), ncol = 1,
...
pc1_Cysto_Clado <- plot_feature_summary((proportion_table_cd %>% select(ASV_58, ASV_20, ASV_24)), metatable_cd$Day, metatable_cd$Tech, bws = 30, nrow = 3)+ labs(x = "Day", y = "Relative Abundance")+
  scale_fill_manual(values = Tech_color)+
  scale_color_manual(values = Tech_color)+
  facet_wrap(vars(feature_all), ncol = 1,
             scales = "free_y", labeller = 
